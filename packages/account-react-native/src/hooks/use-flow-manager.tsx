...
const getSmartAccount = async (owner: Address) => {
  const response = await fetch(`http://192.168.10.43:4001/contract/by-owner/${owner}`);
  if (!response.ok) {
...
const requestConsent = async (accessToken: string, kinds: string[]) => {
  const consentResponse = await fetch(`http://192.168.10.43:4001/me/consent/giveMany`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
