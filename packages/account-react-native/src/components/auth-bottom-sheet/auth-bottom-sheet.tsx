...
import { AuthBottomSheetProps, AuthSheetContextType, AuthBottomSheetStep } from "./types";
...
export function AuthBottomSheet(props: AuthBottomSheetProps) {
  const [stepHistory, setStepHistory] = useState<AuthBottomSheetStep[]>(["signIn"]);

  const goTo = (step: AuthBottomSheetStep) => setStepHistory((prev) => [...prev, step]);
  ...
  return (
    <BottomSheetModalProvider>
      <AuthSheetContext.Provider
        value={{ currentStep: stepHistory[stepHistory.length - 1], goTo, goBack, ...props }}
      >
        <BottomSheet
          onClose={onClose}
          ref={bottomSheetRef}
...
export function useAuthSheet() {
  ...
  if (!context) throw new Error("useAuthSheet must be used within AuthBottomSheetProvider");
  return context;
}
