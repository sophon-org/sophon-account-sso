...
describe('Provider > Handlers > handleSendTransaction', () => {
  it('should return result body if available', async () => {
    ...
    // when
    const result = await handleSendTransaction(sender, params);

    // then
    expect(sender).toHaveBeenCalledWith('eth_sendTransaction', params);
  ...
  it('should throw an error if RPC responded with error', async () => {
    ...
    // when
    const call = () => handleSendTransaction(sender, params);
