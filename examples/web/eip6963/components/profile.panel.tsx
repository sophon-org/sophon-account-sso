...
export const ProfilePanel = () => {
  ...
  const isConnected = wagmiAcc.isConnected || sophonAcc.isConnected || !!sophonAcc.account?.address;

  const address = (wagmiAcc.address ?? sophonAcc.account?.address) as Address | undefined;
  ...
  useEffect(() => {
    const refetch = async () => {
      if (writeContractData) {
        await waitForTransactionReceipt(wagmiConfig, { hash: writeContractData });
        refetchMintMe({});
      }
    };
    refetch();
    ...
    <div className="flex flex-col gap-1 mt-2 w-full">
      <div className="flex flex-col gap-2 border border-gray-300 rounded-md p-4">
        <p className="text-sm text-gray-500">
          <span className="font-bold">User:</span> {address ? shortenAddress(address) : "—"}
        </p>
        <p className="text-sm text-gray-500">
        ...
        <p className="text-sm text-gray-500">
          ...
          {balanceMintMe ? formatUnits(balanceMintMe.value, balanceMintMe.decimals) : "0"}{" "}
          <button className="text-xs text-gray-500 underline" onClick={handleMint} type="button">
            Mint mint
          </button>
        </p>
      </div>
