...
import { useConnectMode, MODES, type ConnectMode } from "./connect-mode";
...
export default function Connectors() {
  ...
  }, [wagmiDisconnect, sophonLogout]);

  if (isConnected) {
    return (
      <div key={mode} className="flex items-center gap-3">
        <span className="flex items-center h-9 rounded border px-3" title={currentAddress}>
  ...
  return (
    <div key={mode} className="flex flex-col gap-3 p-4 border rounded-lg max-w-lg w-full">
        ...
        <PrimaryButton
          onClick={async () => {
            try {
              ...
              alert("Could not open Sophon sign-in. Check console for details.");
            }
          }}
        >
          Sign in with Sophon
      ...
      {mode === "connectkit" && (
        <ConnectKitButton.Custom>
          {({ show }) => <PrimaryButton onClick={show}>Sign in with ConnectKit</PrimaryButton>}
        </ConnectKitButton.Custom>
      )}
      ...
      {mode === "rainbowkit" && (
        <ConnectButton.Custom>
          {({ openConnectModal }) => (
            <PrimaryButton onClick={openConnectModal}>Sign in with RainbowKit</PrimaryButton>
          )}
        </ConnectButton.Custom>
      )}

      {mode === "walletconnect" && (
        <PrimaryButton onClick={openWalletconnectModal}>Sign in with WalletConnect</PrimaryButton>
      )}
    </div>
  );
}
