...
describe("PartnerRegistryService", () => {
  ...
  it("falls back to GET when HEAD 405 and GET 200", async () => {
    ...
    fetchMock
      ...
      .mockResolvedValueOnce(new Response('{}', { status: 200, headers: { "content-type": "application/json" } })); // GET

    const ok = await svc.exists(VALID_ID);
    expect(ok).toBe(true);
    expect(fetchMock).toHaveBeenNthCalledWith(1, url, expect.objectContaining({ method: "HEAD" }));
  ...
  it("returns false (and assertExists throws) when not found (404)", async () => {
    ...
    await expect(svc.assertExists("non-existent")).rejects.toThrow("Audience not allowed: non-existent");
