...
describe('RateLimitGuard', () => {
  ...
  beforeEach(() => {
    ...
    (guard as RateLimitGuard & { logger: { warn: jest.Mock } }).logger = {
      warn: jest.fn(),
    };
  ...
  afterEach(() => {
    ...
    (guard as unknown as { requests: Map<string, unknown> }).requests.clear();
  });
  ...
  describe('canActivate', () => {
    ...
    it('should use forwarded IP from x-forwarded-for header', () => {
      ...
      const clientsMap = (
        guard as unknown as { requests: Map<string, unknown> }
      ).requests;
