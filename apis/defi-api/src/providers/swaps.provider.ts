...
export class SwapsProvider implements ISwapProvider {
  ...
  private transformToSwapRequest(
    request: UnifiedTransactionRequest,
  ): SwapActionRequest {
  ...
  private transformToUnifiedStatusResponse(
    ...
    return {
      ...
      timestamps: {
        initiated: tx.srcTx?.timestamp
          ? new Date(parseInt(tx.srcTx.timestamp) * 1000)
          : undefined,
        confirmed: tx.dstTx?.timestamp
          ? new Date(parseInt(tx.dstTx.timestamp) * 1000)
          : undefined,
  ...
  private calculateProtocolFees(tx: any): string {
    ...
    try {
      ...
      this.loggingService.logProviderDebug(
        this.providerId,
        'Error calculating protocol fees',
        { error: error.message },
      );
      return '0';
    }
  ...
  private calculateTotalFees(tx: any): string {
    ...
    try {
      ...
      const totalFees =
        BigInt(srcGasUsed) + BigInt(dstGasUsed) + BigInt(protocolFees);
